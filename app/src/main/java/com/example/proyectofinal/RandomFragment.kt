        ...
        for (i in 0..4){
            ...
            retroP.enqueue(object : Callback<ProductoEntry> {
                override fun onResponse(call: Call<ProductoEntry>, response: Response<ProductoEntry>) {
                    ...
                    li.set(i,JSONObject("{\"title\": \"${respuesta?.nom.toString()}\", \"price\":\"${respuesta?.precio.toString()}\",\"description\":\"${respuesta?.descripcion.toString()}\",\"image\":\"${respuesta?.imagen.toString()}\"}"),)
                    val d = Log.d("res", li.toString())
                    val adapter = MainAdapter(li)
